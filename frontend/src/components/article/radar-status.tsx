...
export function RadarStatus() {
  ...
  return <>
    <div className="flex flex-wrap items-center justify-between gap-2 border-b px-4 py-2 sm:px-6">
      <div className="text-xs text-muted-foreground"><p>{stale?'更新已延迟 · ':''}最后采集 {date(data.last_success_at)} · 来源 {ok}/{data.sources.length}</p><p className="mt-1">已配置源的部分覆盖 · AI 简报每 6 小时最多更新一次</p></div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        <p className="text-xs text-muted-foreground">公司别名匹配可能遗漏或误判；“全球医药”包括上市市场尚未确定的新闻。简报由 AI 根据标题和摘录生成，请以原文为准。</p>
      </DialogContent>
    </Dialog>
  </>;
}
